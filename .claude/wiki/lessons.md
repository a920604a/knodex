...
## [2026-04-23] ensure_default_tags 用 count > 0 判斷導致預設值永遠不補建

**症狀：**
書卡下拉選單找不到任何預設主題標籤（技術、產品、設計…），但資料庫裡其實已有手動建立的標籤。

**根本原因：**
`ensure_default_tags` 邏輯是「完全沒有任何標籤才 seed」：
```python
count = await db.scalar(select(func.count()).select_from(DocumentTag))
if count and count > 0:
    return  # 只要有任何一個標籤就直接跳出
```
使用者只要曾手動建過一個標籤，11 個預設標籤就永遠不會被寫入。

**教訓：**
seed 函式應逐一比對每個預設值是否已存在，缺的才補建（upsert 思維）：
```python
result = await db.execute(
    select(DocumentTag.name).where(DocumentTag.name.in_(DEFAULTS))
)
existing = {row[0] for row in result.all()}
missing = [n for n in DEFAULTS if n not in existing]
if missing:
    db.add_all(DocumentTag(name=n) for n in missing)
    await db.commit()
```

**關鍵字：** seed, default data, ensure, count > 0, upsert, document_tags

---

## [2026-04-23] asyncio.to_thread + asyncio.run + asyncpg 三層嵌套導致 migration 卡住

**症狀：**
後端啟動後永遠卡在：
```
INFO:app.main:Running DB migrations...
...
**根本原因：**
`asyncio.to_thread(_do_migrate)` 在 thread pool 中跑 `_do_migrate`，
`_do_migrate` 呼叫 `command.upgrade` → env.py → `asyncio.run(run_async_migrations())`，
`run_async_migrations` 內的 `connection.run_sync()` 又透過 `loop.run_in_executor` 把 sync 程式碼放回 thread pool。
三層（asyncio event loop → thread → new event loop → thread pool）嵌套，asyncpg 的連線在底層 thread 中無法正確被喚醒，形成 deadlock。

**教訓：**
alembic 的 async migration 不應用 `asyncio.to_thread` 包裝後再讓 env.py 自行 `asyncio.run()`。
改用 subprocess 完全隔離，最可靠：
```python
...
```
subprocess 有自己的 Python 程序和 event loop，與父程序完全無關。

**關鍵字：** asyncio.to_thread, asyncio.run, asyncpg, alembic, migration, deadlock, lifespan, subprocess
